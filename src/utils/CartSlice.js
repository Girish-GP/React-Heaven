...
const CartSlice = createSlice({
    ...
    reducers:{
        addCartItem: (state, action) => {
            ...
            const itemId = action?.payload?.card?.info?.id ?? action?.payload?.itemData?.card?.info?.id;

            // Find the item in the cart by its unique identifier
            const existingItem = state?.items?.find(item => item?.itemData?.card?.info?.id === itemId);

            if (existingItem) {
                // If the item already exists, update the count and total price
                existingItem.itemCount += 1;
                existingItem.itemTotalPrice += ( Number(action?.payload?.card?.info?.price) ?? Number(action?.payload?.itemData?.card?.info?.price)) / 100;
            } else {
           ...
           // Extract the item ID or any unique identifier from the payload
            ...
            if(existingItem){
              if(existingItem?.itemCount > 1){
                ...
                state.items = state?.items?.filter(obj => obj?.itemData?.card?.info?.id !== itemId)
              }
            } 
        },
