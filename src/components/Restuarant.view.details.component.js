...
const RestuarantViewDetails = () => {
  ...
  // }
  if (dataObj === null) return <Shimmer2Component />;
  const {
    name,
    avgRating,
    costForTwoMessage,
    ...
    sla,
    feeDetails,
  } = dataObj ? dataObj[2]?.card?.card?.info : {};
  const offersList = dataObj
    ? dataObj[3]?.card?.card?.gridElements?.infoWithStyle?.offers
    : [];
  const offersList2 = [offersList[0], offersList[1]];
  const itemCategoriesList =
    dataObj[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards?.filter(
      (obj) =>
        obj?.card?.card["@type"] ===
        "type.googleapis.com/swiggy.presentation.food.v2.ItemCategory"
    );
  const menu =
    dataObj[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[1]?.card?.card
      ?.carousel ??
    dataObj[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[1]?.card?.card
      ?.itemCards;
  return (
        ...
        <div className="flex  border-b-2 w-full mt-4 gap-2">
          {dataObj[1]?.card?.card?.tabs?.map((obj) => {
            return (
              <a
                key={obj?.id}
                className="cursor-pointer font-bold text-lg pe-4 border-b-4 border-b-orange-600"
              >
                {obj?.title}
              </a>
            );
        ...
        <div className="border-none text-base rounded-b-3xl h-52 mt-8 w-full relative ps-4 pb-4 pe-4 box-border bg-slate-50">
          <div className="font-bold absolute p-4 top-0 border-2 w-[calc(100%-2rem)] h-[calc(100%-1rem)] bg-white rounded-2xl">
            <div className="mb-2 flex gap-4 ">
              <p>
                {dataObj[2]?.card?.card?.info?.avgRating} (
                {dataObj[2]?.card?.card?.info?.totalRatingsString})
              </p>
              <p>. {costForTwoMessage}</p>
            </div>
            <p className="text-orange-500 mb-2 underline text-sm decoration-slice">
              {cuisines?.join(", ")}
            </p>
            <p className="mb-2 text-sm">
              Outlet:<span className="px-2">{areaName}</span>
            </p>
        ...
        <div className="flex w-full gap-3 mt-2">
          {offersList2?.map((obj) => {
            return (
              <div
                key={obj?.info?.header}
                className="w-1/3 h-16 rounded-2xl border-2 p-2"
              >
